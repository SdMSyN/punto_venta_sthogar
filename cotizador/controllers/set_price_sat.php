...
$pdf->Cell(70, 7, utf8_decode('Concepto (SAT)'), 1, 0, 'C', true);
    ...
    if ($con->query($sqlInsertPriceInfo) === TRUE) {
        ...
        for ($i = 0; $i < count($_POST['id']); $i++) {
            ...
            if ($con->query($sqlInsertProductPrice) === TRUE) {
                $sqlGetProductInfo = "SELECT img, nombre, descripcion, codigo_sat FROM $tProduct WHERE id='$idProduct' ";
                ...
                $pdf->Cell(70, 30, utf8_decode( $rowGetProductInfo[ 'nombre' ] . ' ( '. $rowGetProductInfo[ 'codigo_sat' ] . ' )' ), 1, 0, 'C', $banColor);
